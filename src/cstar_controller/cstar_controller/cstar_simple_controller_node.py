...
class CStarSimpleControllerNode(Node):
    def __init__(self) -> None:
        ...
        # 普通目标点到达阈值。
        # 注意：controller 的停车阈值必须小于 planner 的 goal_center_tolerance。
        # planner 默认 goal_center_tolerance=0.08，所以这里建议 0.05。
        self.declare_parameter('goal_tolerance', 0.03) #建议0.05

        # escape path 最后一个点的到达阈值。
        # planner 默认 retreat_center_tolerance=0.09，所以这里建议 0.06。
        self.declare_parameter('escape_final_tolerance', 0.06) #建议0.06

        # 普通目标点控制参数
        self.declare_parameter('heading_tolerance', 0.35)
        self.declare_parameter('max_linear_speed_normal', 0.08) #建议0.06 稳定0.08
        self.declare_parameter('max_linear_speed_escape', 0.07) #建议0.05 稳定0.07
        self.declare_parameter('max_angular_speed', 0.25) #建议0.25 

        self.declare_parameter('min_linear_speed', 0.025) #建议0.015
        self.declare_parameter('min_angular_speed', 0.06)

        self.declare_parameter('k_linear', 1.0) #建议0.45
        ...
        self.get_logger().info(
            ...
            f'normal_v={self.max_linear_speed_normal:.2f}, '
            f'escape_v={self.max_linear_speed_escape:.2f}, '
            f'max_w={self.max_angular_speed:.2f}, '
            f'goal_tol={self.goal_tolerance:.2f}, '
            f'escape_final_tol={self.escape_final_tolerance:.2f}'
        )
